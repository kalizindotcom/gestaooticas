...
const featureCatalog: Array<[string, string, LucideIcon]> = [
...
function statusLabel(status: string) {
  const labels: Record<string, string> = { completed: 'Concluído', partial: 'Parcial', failed: 'Falhou', running: 'Executando', imported: 'Importado', restored: 'Restaurado', restore_pending: 'Restauração pendente' };
...
function statusClass(status: string) {
  ...
  if (status === 'partial' || status === 'running' || status === 'restore_pending') return 'border-amber-500/30 bg-amber-500/10 text-amber-600';
...
export default function Settings() {
  ...
  const [operationalMetrics, setOperationalMetrics] = useState<Record<string, any> | null>(null);
  ...
  const loadBackupData = async () => {
    ...
    const integrityResult = await localApi.backupAdmin.integrityLatest();
    if (integrityResult.data) setIntegrityReport(integrityResult.data);
    const metricsResult = await localApi.backupAdmin.metrics();
    if (metricsResult.data) setOperationalMetrics(metricsResult.data);
  };
  ...
  const runIntegrityCheck = async () => {
    ...
    const result = await localApi.backupAdmin.runIntegrityCheck();
    setIntegrityLoading(false);
    if (result.error) return toast.error(result.error.message);
    if (result.data) setIntegrityReport(result.data);
    const counts = result.data?.counts || {};
    if (Number(counts.critical || 0) > 0) toast.error(`Verificação encontrou ${counts.critical} problema(s) crítico(s).`);
    else if (Number(counts.warning || 0) > 0) toast.warning(`Verificação concluída com ${counts.warning} alerta(s).`);
    else toast.success('Integridade confirmada: nenhum vínculo inválido encontrado.');
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="backup" className="mt-6 space-y-6">
          ...
          <Card className="premium-shadow border-border/60"><CardHeader><CardTitle className="flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between"><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /> Integridade dos dados</span><Button variant="outline" size="sm" onClick={runIntegrityCheck} disabled={integrityLoading} className="gap-1.5"><RefreshCw className={`h-3.5 w-3.5 ${integrityLoading ? 'animate-spin' : ''}`} /> {integrityLoading ? 'Verificando...' : 'Verificar agora'}</Button></CardTitle></CardHeader><CardContent>{integrityReport ? <div className="space-y-4"><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Status</p><p className={`mt-1 text-sm font-bold ${integrityReport.ok ? 'text-emerald-600' : 'text-red-600'}`}>{integrityReport.ok ? 'Saudável' : 'Atenção necessária'}</p></div><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Registros</p><p className="mt-1 text-lg font-black">{integrityReport.counts?.rows || 0}</p></div><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Críticos</p><p className="mt-1 text-lg font-black text-red-600">{integrityReport.counts?.critical || 0}</p></div><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Alertas</p><p className="mt-1 text-lg font-black text-amber-600">{integrityReport.counts?.warning || 0}</p></div></div>{Number(integrityReport.counts?.issues || 0) > 0 ? <div className="space-y-2">{(integrityReport.issues || []).slice(0, 5).map((issue: any, index: number) => <div key={`${issue.code}-${issue.row_id}-${index}`} className="rounded-xl border border-border/60 bg-accent/20 p-3"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className={issue.severity === 'critical' ? 'border-red-500/30 text-red-600' : 'border-amber-500/30 text-amber-600'}>{issue.severity === 'critical' ? 'Crítico' : 'Alerta'}</Badge><span className="text-xs font-semibold">{issue.table}{issue.row_id ? ` · ${issue.row_id}` : ''}</span></div><p className="mt-1 text-xs text-muted-foreground">{issue.message}</p></div>)}{integrityReport.issues.length > 5 && <p className="text-xs text-muted-foreground">Mostrando 5 de {integrityReport.issues.length} ocorrências. Execute uma verificação detalhada pelo histórico da API.</p>}</div> : <p className="text-sm text-muted-foreground">Nenhum vínculo inválido detectado entre empresas, lojas, clientes, vendas, O.S., estoque, financeiro e fiscal.</p>}<p className="text-[11px] text-muted-foreground">Última verificação: {formatDate(integrityReport.checked_at)}</p></div> : <p className="text-sm text-muted-foreground">Ainda não há relatório. Execute a primeira verificação antes de uma restauração.</p>}</CardContent></Card>
          <Card className="premium-shadow border-border/60"><CardHeader><CardTitle className="flex items-center gap-2 text-sm"><BarChart3 className="h-4 w-4 text-primary" /> Saúde operacional</CardTitle></CardHeader><CardContent>{operationalMetrics ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Requisições desde o boot</p><p className="mt-1 text-lg font-black">{operationalMetrics.requests?.total || 0}</p><p className="text-[11px] text-muted-foreground">Média {Number(operationalMetrics.requests?.average_duration_ms || 0).toFixed(0)} ms</p></div><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Memória do processo</p><p className="mt-1 text-lg font-black">{formatBytes(operationalMetrics.process?.memory_rss_bytes)}</p><p className="text-[11px] text-muted-foreground">Uptime {operationalMetrics.uptime_seconds || 0}s</p></div><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Último backup</p><p className="mt-1 text-sm font-bold">{statusLabel(String(operationalMetrics.backups?.latest_job?.status || 'não executado'))}</p><p className="text-[11px] text-muted-foreground">{formatDate(operationalMetrics.backups?.latest_job?.completed_at || operationalMetrics.backups?.latest_job?.created_at)}</p></div><div className="rounded-xl border border-border/60 p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Último restore</p><p className="mt-1 text-sm font-bold">{operationalMetrics.last_restore_completed ? 'Retomado com sucesso' : 'Nenhum restore recente'}</p><p className="text-[11px] text-muted-foreground">{formatDate(operationalMetrics.last_restore_completed?.restored_at)}</p></div></div> : <p className="text-sm text-muted-foreground">Métricas operacionais indisponíveis no momento.</p>}</CardContent></Card>
