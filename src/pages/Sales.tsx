  ...
  return (
    <div className="space-y-6 pb-10 animate-in fade-in duration-300">
      <PageHeader
        ...
        actions={(
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" onClick={() => void refreshSales()} className="h-10 gap-2 border-border/70 bg-card">
              <RefreshCw className="h-4 w-4" /> Atualizar
            </Button>
            <PermissionGate module="sales" action="create">
              <Button onClick={openNewSale} className="h-10 gap-2 shadow-sm">
        ...
        <CardContent className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Os indicadores acompanham os filtros aplicados e o escopo global.</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-1.5 rounded-xl bg-muted/60 p-1">
            {[
              ...
              <Button key={value} variant="ghost" size="sm" onClick={() => setDatePreset(value as any)} className={cn('h-8 rounded-lg px-3 text-xs', value === 'all' && !dateFrom && !dateTo ? 'bg-card text-primary shadow-sm' : 'text-muted-foreground')}>
                {label}
              </Button>
            ))}
          </div>
        ...
        <CardHeader className="gap-4 border-b border-border/60 pb-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <CardDescription>Pesquise, filtre e abra qualquer venda para conferir os detalhes.</CardDescription>
            </div>
            <div className="flex items-center gap-1 rounded-lg bg-muted/60 p-1">
              <Button variant="ghost" size="sm" onClick={() => setViewMode('table')} className={cn('h-8 gap-1.5 px-3 text-xs', viewMode === 'table' && 'bg-card text-primary shadow-sm')}><List className="h-3.5 w-3.5" /> Tabela</Button>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(220px,1.4fr)_150px_150px_150px_150px_150px_auto]">
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input value={searchValue} onChange={(event) => setSearchValue(event.target.value)} placeholder="Buscar venda, cliente, documento ou vendedor..." className="h-10 pl-9" />
            </div>
      ...
      <Dialog open={isNewSaleOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="flex h-[min(94vh,900px)] max-w-6xl flex-col gap-0 overflow-hidden border-border/80 bg-background p-0 shadow-2xl">
          <DialogHeader className="shrink-0 border-b border-border/70 bg-card px-5 py-4 sm:px-7">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">{([['1', 'Contexto', 'Cliente e loja'], ['2', 'Produtos', `${saleItems.length} item(s)`], ['3', 'Revisão', 'Pagamento e total']] as const).map(([step, title, helper]) => <button key={step} type="button" onClick={() => { const target = Number(step) as SaleStep; if (target < saleStep || (target === 2 && validateStep(1)) || (target === 3 && validateStep(1) && validateStep(2))) setSaleStep(target); }} className={cn('rounded-xl border px-3 py-2 text-left transition-colors', saleStep === Number(step) ? 'border-primary/40 bg-primary/10' : 'border-border/60 bg-muted/20 hover:border-primary/30')}><div className="flex items-center gap-2"><span className={cn('flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold', saleStep === Number(step) ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground')}>{step}</span><span className={cn('text-xs font-bold', saleStep === Number(step) ? 'text-primary' : 'text-foreground')}>{title}</span></div><p className="mt-1 pl-8 text-[10px] text-muted-foreground">{helper}</p></button>)}</div>
          </DialogHeader>

          <ScrollArea className="min-h-0 flex-1"><div className="mx-auto w-full max-w-5xl space-y-6 p-5 sm:p-7">
            ...
            {saleStep === 2 && <div className="space-y-6">
              ...
              <div className="grid gap-3 sm:grid-cols-3"><SummaryTile label="Subtotal" value={currency(subtotal)} /><SummaryTile label="Produtos" value={`${saleItems.length} tipo(s)`} /><SummaryTile label="Unidades" value={String(saleItems.reduce((sum, item) => sum + item.qty, 0))} /></div>
            </div>}
...
function MetricCard({ label, value, helper, icon: Icon, accent = false }: { label: string; value: string; helper: string; icon: any; accent?: boolean }) {
  return <Card className="border-border/70 bg-card shadow-sm"><CardContent className="flex min-h-[108px] flex-col justify-between gap-2 p-4"><div className="flex items-center justify-between gap-2"><div className={cn('flex h-8 w-8 items-center justify-center rounded-xl', accent ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground')}><Icon className="h-4 w-4" /></div><span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{label}</span></div><div><p className={cn('truncate text-lg font-black', accent ? 'text-primary' : 'text-foreground')}>{value}</p><p className="mt-0.5 truncate text-[10px] text-muted-foreground">{helper}</p></div></CardContent></Card>;
}

function SectionHeading({ icon: Icon, title, description }: { icon: any; title: string; description: string }) {
  return <div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="h-4 w-4" /></div><div><h3 className="text-sm font-bold text-foreground">{title}</h3><p className="mt-0.5 text-xs text-muted-foreground">{description}</p></div></div>;
}
