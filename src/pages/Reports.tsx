...
export default function Reports() {
  ...
  const productMix = useMemo(() => { const map: Record<string, any> = {}; filteredSales.forEach((sale: any) => (sale.items || []).forEach((item: any) => { const product = productById.get(item.productId); const key = item.product || product?.name || 'Produto sem nome'; if (!map[key]) map[key] = { name: key, revenue: 0, units: 0, category: product?.category || 'Sem categoria', brand: product?.brand || 'Sem marca' }; map[key].revenue += Number(item.total || item.total_price || 0); map[key].units += Number(item.qty || item.quantity || 0); })); return Object.values(map).sort((a: any, b: any) => b.revenue - a.revenue); }, [filteredSales, productById]);
